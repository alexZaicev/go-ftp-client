...
import (
	...
	parsers "github.com/alexZaicev/go-ftp-client/internal/adapters/parsers"
)
...
// Parse provides a mock function with given fields: data, options
func (_m *Parser) Parse(data string, options *parsers.Options) (*entities.Entry, error) {
	ret := _m.Called(data, options)

	var r0 *entities.Entry
	if rf, ok := ret.Get(0).(func(string, *parsers.Options) *entities.Entry); ok {
		r0 = rf(data, options)
	} else {
		if ret.Get(0) != nil {
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(string, *parsers.Options) error); ok {
		r1 = rf(data, options)
	} else {
		r1 = ret.Error(1)
	}
